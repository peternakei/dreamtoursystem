...
onMounted(async () => {
  try {
    counts.value = (await api.get('/workspace/dashboard')).data.counts
  } catch (e) {
  ...
  <MainLayout title="Dashboard">
    ...
    <Card class="mt-6">
      ...
      <CardContent class="flex flex-wrap gap-3">
        <RouterLink v-for="item in ['trips','destinations','accommodations','vehicles','quotations']" :key="item"
                    :to="'/'+item+'/list'">
          <Button variant="outline" class="capitalize">{{ item }}
            <ArrowUpRight class="h-4 w-4"/>
          </Button>
        </RouterLink>
      </CardContent>
