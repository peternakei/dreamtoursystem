                                ...
                                <table id="fixed-header-datatable"
                                    ...
                                    <tbody>
                                        @foreach ($ratings as $rate)
                                            <tr>
                                                <td>{{ $rate->tourist->name }}</td>
                                                <td>{{ Str::limit($rate->trip->trip_code,80,'...') }}</td>
                                                <td>{{ Str::limit($rate->trip->name,80,'...') }}</td>
                                                <td>{{ Str::limit($rate->destination->name,80,'...') }}</td>
                                                <td>{{ $trip->rating }}</td>
                                                <td>{{ $trip->created_at }}</td>
                                                <td class="table-action">
                                                    <a href="{{ route('ratings.show', $rate->uuid) }}"
