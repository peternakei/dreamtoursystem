<div id="change-trip-status-modal" class="modal fade" tabindex="-1" role="dialog"
    aria-labelledby="standard-modalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="standard-modalLabel">Change <span
                        class="text-primary">{!! $trip->name !!}</span>`s Status</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
            </div>
            <form action="{{ route('trips.change_status', $trip->uuid) }}"
                id="changeTripStatusForm" name="changeTripStatusForm" method="POST">
                @csrf
                <div class="modal-body">
                    <h5 style="font-weight: 500;">Fill the form to change trip status</h5>
                    <div class="row mt-3">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <select class="form-control select2" id="current_status" name="current_status"
                                            ...
                                            <optgroup label="Current Status">
                                                ...
                                                <option value="{{ $trip->is_active }}" selected>
                                                    @if ($trip->is_active) Active
                                                    @else
                                                        Inactive @endif
                                                </option>
                                            </optgroup>
                ...
                <div class="modal-footer">
                    <button type="button" onclick="submitCreateForm('changeTripStatusForm')"
